...
const Swimmer: React.FC = () => {
  ...
  const { loading, error, data, refetch: _ } = useQuery(
  ...
  let swimmers = data.getSwimmersByName;

  return (
    <div className="max-w-screen-xl mx-auto mt-8">
      {/* Search bar at the top */}
      <div className="p-1 bg-gray-100 mb-4 flex items-center rounded-md">
        <input
          type="text"
          value={searchTerm}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
        {swimmers.length === 0 ? (
          <h1 className="text-4xl font-bold mb-4 text-white">No Swimmers found</h1>
          ...
          swimmers.map((swimmer: any, index: number) => (
            <div key={index} className="bg-white shadow-md p-4 rounded-lg mb-4">
              <div className="flex flex-col items-start justify-between h-full">
                <div>
                  <p className="text-lg font-semibold text-blue-700">ID: {swimmer.id}</p>
                  <p className="text-lg font-semibold text-blue-700">Name: {swimmer.name}</p>
                  <p className="text-lg font-semibold text-blue-700">Team: {swimmer.team}</p>
